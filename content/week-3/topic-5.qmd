...
You need certain R **packages for using R Markdown** and also for carrying out this tutorial.
The following instructions will help to get you set up and ready to go.
...
If you are asked whether you would like to **restart R prior to installing**, click on **Yes**.
(If it immediately asks again, press **No**.)
...
We've installed the package called  tinytex , but we also needed to run its function install_tinytex() to install Tex on your computer.
So the tinytex package helps us install Tex, which is required in order to produce PDF outputs. 
 
...
Once this has run, you might see a message to reopen the R session and check if  tinytex::is_tinytex()  is TRUE. 
Run this bit of code in the Console to check and hopefully it will return TRUE.
...
Hooray! 
You have installed all the relevant packages.
