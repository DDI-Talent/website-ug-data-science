...
In this case we do want to track both files, so tick **both check-boxes** and the ![untracked yellow question mark](images/git-r/untracked.png) should become green squares with `A` inside ![added green A](images/git-r/added.png).
...
Once again, take a look at the Git pane. We should see that the `.gitignore` file has appeared again. This time instead of ![untracked yellow question mark](images/git-r/untracked.png), it has a blue square with `M` inside ![modified blue M](images/git-r/modified.png) inside. This is Git saying
