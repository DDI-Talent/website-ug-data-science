...
![`.DS_Store` files often crop up in repos. They are Mac only files and they are a nuisance for your repo. Add them to your `.gitignore` **every time**. Windows has a similar file (`thumbs.db`), but it seems to be less pervasive.](images/git-r/09_rs_gitignore.png){#fig-git-ignore width="50%"}
...
![The **fork** button in the GutHub navbar. Located just above the green **code** button we use to **clone** a repo.](images/git-r/12_gh_forking.png){#fig-forking}
