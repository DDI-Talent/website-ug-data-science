...
It works fine, right?
Well, kind of, for certain simple use-cases.
But there are some limitations for a project scenario.

- Hard to find __specific__ files, especially if you downloaded them a while ago.
- Files are not grouped, so files form different projects end up lumped together.
...
Even bigger issues that using `Downloads`!
such as...

- Still a dumping ground. It's just a different pile.
- Projects get mixed together, and it's easy to accidentally open or overwrite the wrong file.
- Code that refers to files on your Desktop won't work for others, or even for you if you move the project.
- Harder to back up just one project or share it cleanly.

Its also worth noting that on some systems, especially those with cloud sync tools like OneDrive or iCloud, the Desktop folder might be constantly syncing.  
...
![Possible directory structure. In this example, each tutorial is placed in its owdn directory. And all tutorials ar contained in a data science tutorials directory. And that lives sensible in the Documents folder (not on the Desktop).](images/ug-dir-str-small.png){#fig-dir-str fig-align=left}
...
Instead of going all the way to `C:/` it says "starting form here go...".
...
If we attach the package in the usual way `library(here)`, instead of typing
...
In one of your own projects try
