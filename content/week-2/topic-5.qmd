...
Just don't have its syncing with your desktop.
...
![Possible directory structure. In this example, each tutorial is placed in its own directory. And all tutorials ar contained in a data science tutorials directory. And that lives sensible in the Documents folder (not on the Desktop).](images/ug-dir-str-small.png){#fig-dir-str fig-align=left}
...
Notice the use of `::`.
This means we can use the `here` function from within the `here` package, without having to attach the package to our R environment (i.e. no need for `library(here)`K)
